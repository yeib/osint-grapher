...
compute_network_metrics <- function(g) {
  ...
  if (vcount(g) >= 2) {
    ...
    # cluster_louvain falla si hay pesos negativos. Tomamos el valor absoluto para la métrica.
    if ("weight" %in% edge_attr_names(g_undirected)) {
      E(g_undirected)$weight <- abs(E(g_undirected)$weight)
    }
